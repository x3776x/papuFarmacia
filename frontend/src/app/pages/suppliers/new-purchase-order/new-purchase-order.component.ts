...
export class CreateOrderComponent implements OnInit {
  ...
  onSubmit() {
    if (this.form.invalid) {
      this.customDialogService.warning('Por favor completa los campos correctamente');
      return;
    }
    ...
    this.orderService.createOrder(payload).subscribe({
      next: () => {
        this.customDialogService.success('Orden creada con éxito');
        this.router.navigate(['/suppliers']);
      },
      error: (err) => {
        console.error('Error al crear orden:', err);
        this.customDialogService.error('Error al crear la orden. Revisa la consola.');
      },
    });
  }
}
